...
if(strcmp(data,root->data)<=0){
	root->left=bst_add_num(root->left,data);
	...
	root->right=bst_add_num(root->right,data);
...
node* delete_node(node* root,char* data){
	if(root==NULL)return NULL;
		if(strcmp(root->data,data)==0)return operate_on(root);
		else
		if(strcmp(data,root->data)<0){
...
root->left=any_node_to_up(root->left,data);
root->right=any_node_to_up(root->right,data);

	if(root->left!=NULL && strcmp(root->left->data,data)==0)
		return right_rotation(root);
	else
	if(root->right!=NULL && strcmp(root->right->data,data)==0)
